...
--
-- Host: 127.0.0.1
-- Czas generowania: 10 Sty 2024, 13:26
...
--
-- Baza danych: `moja_strona_164333`
--
...
(3, 'wedka', 'wedka savage gear spin 221cm ', '2024-01-03 12:53:07', '2024-01-10 13:15:34', '0000-00-00', '600.00', '8.00', 0, 'Niedostępny', 2, 'średni', 'https://pleciona.pl/userdata/public/gfx/107917/Savage-Gear-Revenge-SG6-2cz-S.jpg'),
(4, 'dss', 'dsad', '2024-01-10 13:00:41', '2024-01-10 13:01:14', '2024-01-31', '123.00', '5.00', 123, 'Dostępny', 2, 'maly', 'expamle');

--
-- Wyzwalacze `produkty`
--
DELIMITER $$
CREATE TRIGGER `przed_insert_produkt` BEFORE INSERT ON `produkty` FOR EACH ROW SET NEW.status_dostepnosci = CASE 
    WHEN NEW.ilosc_dostepnych_sztuk > 0 AND (NEW.data_wygasniecia IS NULL OR NEW.data_wygasniecia > NOW()) 
    THEN 'Dostępny'
    ELSE 'Niedostępny'
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `przed_update_produkt` BEFORE UPDATE ON `produkty` FOR EACH ROW SET NEW.status_dostepnosci = CASE 
    WHEN NEW.ilosc_dostepnych_sztuk > 0 AND (NEW.data_wygasniecia IS NULL OR NEW.data_wygasniecia > NOW()) 
    THEN 'Dostępny'
    ELSE 'Niedostępny'
END
