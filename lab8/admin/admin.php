...
function EdytujPodstrone($id)
{
    ...
    <form action="" method="post" style="text-align: center">
        ...
        <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($page_title); ?>"
               required>
        <p></p>
        <label for="content">Treść:</label>
        <p></p>
        <textarea id="content" name="content" rows="20" cols="100"
                  required><?php echo htmlspecialchars($page_content); ?></textarea>
        <p></p>
        <label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save'])) {
        ...
        echo 'Debug Info: ' . $title . ' ' . $content . ' ' . $status . ' ' . $id;
        $conn = db_connect();
